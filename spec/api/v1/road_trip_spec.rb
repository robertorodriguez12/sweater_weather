...
RSpec.describe "Road Trip" do
  before :each do
    ...
    it 'cannot create a roadtrip with missing fields' do
      params = {origin: "Denver,CO",
                destination: "Pueblo,CO",
                api_key: @api_key}
      post "/api/v1/road_trip?origin=#{params[:origin]}&api_key=#{params[:api_key]}"

      expect(response.status).to eq(401)
      expect(response.body).to eq("You are missing a required field")
    end

    it 'cannot create a roadtrip without an API key' do
      params = {origin: "Denver,CO",
                destination: "Pueblo,CO",
                api_key: @api_key}
      post "/api/v1/road_trip?origin=#{params[:origin]}&destination=#{params[:destination]}"
    ...
    it 'cannot create an impossible roadtrip' do
      params = {origin: "Denver,CO",
                destination: "London",
                api_key: @api_key}
      post "/api/v1/road_trip?origin=#{params[:origin]}&destination=#{params[:destination]}&api_key=#{params[:api_key]}"
